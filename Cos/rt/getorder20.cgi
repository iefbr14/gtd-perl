...
#
#	on the server type: perl getorder20.cgi order=2884553
#
#######################################
use encoding "utf-8";
use DBI;
...
$ref->{trace_size} = length($ref->{trace_value});
...
foreach $key (sort keys %$ref) {
	...
	if ( $key =~ /extra_info/ ) {
		$key =~ s/extra_info=//;
		print "$value";
		foreach my $extra_fields (split("\n",$value)) {
			$extra_fields =~  m/^([^:]*):\s*(.*)/;
			my ($extraKey, $extraVal) = ($1, $2);
			$ref->{$extraKey} = $extraVal;
		}
		next;
	}
...
# rd codes
    ...
    elsif (equalsIgnoreCase($items->{fdSource}, "UNCUT") && 
    		defined ($ref->{x_uncut_by_diam}))      							{ $description = "UNCUT BY DIAMETER"; }
