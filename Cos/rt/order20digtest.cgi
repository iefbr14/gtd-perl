...
#
#	name: order.cgi
#	purpose:  OpticalOnline HistoryPanel.java use 
...
if (defined ($r->{extra_info})) {
    ...
    if (defined $e->{cbo_eye}) {
    	$r->{frame_eye} = $e->{cbo_eye};
	}
    if (defined $e->{cbo_bridge}) {
    	$r->{frame_bridge} = $e->{cbo_bridge};
	}
}
...
viewpANG_od:		$r->{viewpANG_od}
viewpDIST_os:		$r->{viewpDIST_os}
viewpDIST_od:		$r->{viewpDIST_od}
rvd_os:		$r->{rvd_os}
rvd_od:		$r->{rvd_od}
bvd_os:		$r->{bvd_os}
bvd_od:		$r->{bvd_od}
...
ztilt_os:		$r->{ztilt_os}
ztilt_od:		$r->{ztilt_od}
cllife_os:		$r->{cllife_os}
cllnam_os:		$r->{cllnam_os}
