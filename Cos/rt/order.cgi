...
#rx_od_near 	left  mono near 
#rx_os_near	right mono near
#rx_od_far	left  mono far
#rx_os_far      right mono far
...
if ($mono =~ /^[sm]$/) {	# 1.5 standard data mode
    ...
    $r->{rx_OD_Mono_PD} = $r->{rx_od_far};	# legacy meaning
    $r->{rx_OS_Mono_PD} = $r->{rx_os_far};

    $r->{rx_OS_Far_PD}	= $r->{rx_od_far} + $r->{rx_os_far};
    $r->{rx_OD_Far_PD}	= $r->{rx_od_far} + $r->{rx_os_far};

    $r->{rx_OD_Near_PD} = $r->{rx_od_near} + $r->{rx_os_near};
    $r->{rx_OS_Near_PD} = $r->{rx_od_near} + $r->{rx_os_near};

} elsif ($mono =~ /^[SM]$/) {	# 1.4 data mode
    $r->{rx_od_near} = $r->{rx_OD_Near_PD}/2;
    $r->{rx_os_near} = $r->{rx_OS_Near_PD}/2;

    $r->{rx_od_far}  = $r->{rx_OD_Mono_PD};
    $r->{rx_os_far}  = $r->{rx_OS_Mono_PD};

} elsif ($mono eq 'BPD') {	# legacy data
    $r->{rx_od_near}  = $r->{rx_OD_Near_PD}/2;
    $r->{rx_os_near}  = $r->{rx_OS_Near_PD}/2;

    $r->{rx_od_far}  = $r->{rx_OD_Far_PD}/2;
    $r->{rx_os_far}  = $r->{rx_OS_Far_PD}/2;
...
#Ver. 1.5.7 - deprecate use of lens_SV_MF, it is not used in std.pm
} else {
    $r->{rx_od_near} = $r->{rx_OD_Near_PD};
    $r->{rx_os_near} = $r->{rx_OS_Near_PD};
