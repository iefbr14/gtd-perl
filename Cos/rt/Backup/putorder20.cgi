...
#### if cllnam goes in all Digital data goes in
if (defined $vars->{digitalData} && $vars->{digitalData} eq "true" ) {
	if (defined $vars->{cllnam} && $vars->{cllnam} ne '') {
		$extraInfo .= "cllnam:$vars->{cllnam}\n";
	}
	
	if (defined $vars->{cllifes} && $vars->{cllifes} ne '') {
		$extraInfo .= "cllifes:$vars->{cllifes}\n";
	}
	if (defined $vars->{vxo} && $vars->{vxo} ne '') {
		$extraInfo .= "vxo:$vars->{vxo}\n";
	}
	if (defined $vars->{vxn} && $vars->{vxn} ne '') {
		$extraInfo .= "vxn:$vars->{vxn}\n";
	}
	if (defined $vars->{pnt} && $vars->{pnt} ne '') {
		$extraInfo .= "pnt:$vars->{pnt}\n";
	}
	if (defined $vars->{faf} && $vars->{faf} ne '') {
		$extraInfo .= "faf:$vars->{faf}\n";
	}
	if (defined $vars->{viewpDIST} && $vars->{viewpDIST} ne '') {
		$extraInfo .= "viewpDIST:$vars->{viewpDIST}\n";
	}
	if (defined $vars->{viewpANG} && $vars->{viewpANG} ne '') {
		$extraInfo .= "viewpANG:$vars->{viewpANG}\n";
	}
}
####
