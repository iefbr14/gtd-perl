...
func merge_project() { /*?
	...
	//##	Merge: description
		...
		if ($master->get_title() != $slave->get_title()) {
			if ($desc) {
				$desc .= "\n" . '-'x30
			}
			$desc .= "\n" . $slave->get_title() ."\n"
...
//TODO change to using values, not the reference
func merge_date() { /*?				// keep earliest date
		my($ref, $slave, $key) = @_
...
//TODO change to using values, not the reference
func merge_completed() { /*?				// keep earliest date
		my($ref, $slave) = @_
...
//TODO change to using values, not the reference
func merge_first() { /*?				// update item's importance
		my($ref, $slave, $key) = @_
...
//TODO change to using values, not the reference
func merge_tag() { /*?				// update item's importance
		my($ref, $slave, $key) = @_
